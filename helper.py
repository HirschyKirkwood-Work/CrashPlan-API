...
def get_machines(UID):
	...
	machine_response = SDK.devices.get_all(active=True, page_size=20000) # Page size limits are stupid, but necessary, lol
...
def get_users():
	user_response = SDK.users.get_all(active=True) # Could add page size here, but as of now, not required. Maybe DSP will get massive.
...
def return_dept_dict(cp_all, dept_all): # Returns dictionary with username/userUid combo for users w/ accounts in dept.
...
def user_machine_status(UID): # I could probably integrate return_no_backup() with this. #TODO
...
def sed(file, pattern, repl): # Sed-like replace function. Thank you to David Miller on Stack Overflow https://stackoverflow.com/questions/4427542/how-to-do-sed-like-text-replace-with-python
	with open(file, "r") as sources: # I think I've done similar stuff in the past with nested withs lol. I didn't realize your variables were accessible outside of the with.
